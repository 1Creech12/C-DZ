...
bool deleteSongFile(const string& filename) {
    if (!fileExists(filename)) {
        cout << "❌ Файл '" << filename << "' не найден!\n";
        return false;
    }
    
    cout << "📄 Файл для удаления: " << filename << endl;
    cout << "Вы уверены, что хотите удалить этот файл? (y/n): ";
    char confirmation;
    cin >> confirmation;
    cin.ignore();
    
    if (confirmation != 'y' && confirmation != 'Y') {
        cout << "🚫 Удаление отменено пользователем.\n";
        return false;
    }
    ...
    if (remove(filename.c_str()) == 0) {
        ...
        cout << "❌ Ошибка при удалении файла '" << filename << "'\n";
        return false;
    }
}

// Функция для удаления файла с возможностью выбора из списка
bool deleteFromList() {
    cout << "\n📁 Список доступных файлов в текущей директории:\n";
    cout << "----------------------------------------\n";
    
    // Команда для вывода файлов с расширением .txt
    #ifdef _WIN32
        system("dir *.txt /B");
    #else
        system("ls -1 *.txt 2>/dev/null | head -10");
    #endif
    ...
    return deleteSongFile(filename);
}
...
int main() {
    ...
    cout << "3. 🗑️ Удалить файл с песней\n";
    cout << "4. 🚪 Выход\n\n";
    ...
    while (getline(file, line)) {
        ...
        else if (line.find("Год:") != string::npos) {
            cout << Colors::BRIGHT_YELLOW << "📅 " << line.substr(4) << Colors::RESET << endl;
        }
        else {
